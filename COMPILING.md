...
Install JDK 17, 21, and 25 before building; different compatibility modules use different toolchains. Then run
`./gradlew prepareDependencies build`. It's not uncommon for it to take 15
minutes the first time, since it needs to build two versions of Spigot. When it's done, you can find the resulting jar
at `build/libs/TwiCosmetics-<version>-<buildtype>.jar`. Subsequent builds should take less than a minute.
