...
public class SubCommandHelp extends SubCommand {
    ...
    public SubCommandHelp(TwiCosmetics ultraCosmetics, CommandManager commandManager) {
        super("help", "Shows available TwiCosmetics commands", "", ultraCosmetics);
    ...
    public void showHelp(CommandSender sender, int page) {
        ...
        sender.sendMessage("");
        sender.sendMessage(ChatColor.WHITE + "" + ChatColor.BOLD + "TwiCosmetics Help (/uc <page>) " + ChatColor.DARK_GRAY + "" + ChatColor.BOLD + "(" + page + "/" + getMaxPages(available.size()) + ")");
        int from = 8 * (page - 1);
        int to = 8 * page;
