...
import com.siberanka.twicosmetics.config.*;
...
import com.siberanka.twicosmetics.hook.*;
import com.siberanka.twicosmetics.listeners.*;
import com.siberanka.twicosmetics.menu.CosmeticsInventoryHolder;
import com.siberanka.twicosmetics.menu.Menus;
import com.siberanka.twicosmetics.menu.menus.CustomMainMenu;
import com.siberanka.twicosmetics.mysql.MySqlConnectionManager;
import com.siberanka.twicosmetics.paper.DummyPaperSupport;
import com.siberanka.twicosmetics.paper.PaperSupport;
...
import com.siberanka.twicosmetics.player.profile.ProfileSaveService;
import com.siberanka.twicosmetics.run.FallDamageManager;
import com.siberanka.twicosmetics.run.InvalidWorldChecker;
import com.siberanka.twicosmetics.run.VanishChecker;
import com.siberanka.twicosmetics.treasurechests.TreasureChestManager;
import com.siberanka.twicosmetics.util.*;
import com.siberanka.twicosmetics.util.SmartLogger.LogLevel;
import com.siberanka.twicosmetics.version.ServerVersion;
import com.siberanka.twicosmetics.worldguard.WorldGuardManager;
 ...
 * @since 08-03-2015
 */
public class TwiCosmetics extends JavaPlugin {
    /**
    ...
    public void onLoad() {
        ...
        TwiCosmeticsData.init(this);

        Problem problem = TwiCosmeticsData.get().checkServerVersion();
        if (problem != null) {
    ...
    /**
     * Copies an existing UltraCosmetics data directory on the first TwiCosmetics start.
     * Existing destination files are never overwritten and the legacy directory is kept
     * intact, allowing server owners to roll back without losing player data.
     */
    private void migrateLegacyDataDirectory() {
        Path destination = getDataFolder().toPath();
        Path marker = destination.resolve(".legacy-migration-complete");
        Path parent = destination.getParent();
        if (parent == null || Files.exists(marker)) {
            return;
        }

        Path legacy = parent.resolve("UltraCosmetics");
        if (!Files.isDirectory(legacy) || legacy.equals(destination)) {
            return;
        }
        ...
        try (var paths = Files.walk(legacy)) {
            ...
            Files.writeString(marker, "Migrated from UltraCosmetics; legacy files were preserved.\n");
            getLogger().info("Copied legacy UltraCosmetics data into TwiCosmetics.");
        } catch (IOException | UncheckedIOException exception) {
            getLogger().log(java.util.logging.Level.SEVERE,
                    "Could not migrate the legacy UltraCosmetics data directory", exception);
        }
    }
    ...
    public void start(boolean firstRun) {
        ...
        if (TwiCosmeticsData.get().getServerVersion() == null) {
            getSmartLogger().write(LogLevel.ERROR,
                    "Plugin load has failed, please check earlier in the log for details.");
            return;
        }

        TwiCosmeticsData.get().initConfigFields();

        String langFileName = "messages_" + TwiCosmeticsData.get().getLanguage() + ".yml";
        ...
        if (activeProblems.contains(Problem.BAD_MC_VERSION)) {
            ...
            getSmartLogger().write(LogLevel.WARNING,
                    "This server version is unknown (" + ServerVersion.getMinecraftVersion() +
                            "), but TwiCosmetics will try to continue running.");
        }

        // Initialize NMS Module
        if (!TwiCosmeticsData.get().initModule()) {
        ...
        if (SettingsManager.getConfig().getBoolean("Categories-Enabled." + Category.MORPHS.getConfigPath())) {
            if (!Bukkit.getPluginManager().isPluginEnabled("LibsDisguises")) {
                ...
                // The unavailable optional integration is represented by MOBCHIP_ERROR below.
            } else {
                try {
        ...
        if (!TwiCosmeticsData.get().isMobChipAvailable()) {
            getSmartLogger().write();
        ...
        getSmartLogger().write();
        getSmartLogger().write(
                "TwiCosmetics successfully finished loading in " + (System.currentTimeMillis() - startTime) + "ms!");
        getSmartLogger().write("-------------------------------------------------------------------");
        enableFinished = true;
    }
    ...
    private boolean loadPaperSupport() {
        ...
        try {
            paperSupport =
                    Class.forName("com.siberanka.twicosmetics.paper.PaperSupportImpl").asSubclass(PaperSupport.class)
    ...
    public void shutdown() {
        ...
        if (mySqlConnectionManager != null && mySqlConnectionManager.success()
                && !playerManager.flushSqlProfiles(Duration.ofSeconds(2))) {
    ...
    private void setupEconomy() {
        ...
        TwiCosmeticsData.get().checkTreasureChests();
    }

    private void setupMetrics() {
        Metrics metrics = new Metrics(this, 2629);
        String nms;
        ServerVersion sv = TwiCosmeticsData.get().getServerVersion();
        if (TwiCosmeticsData.get().getVersionManager().isUsingNMS()) {
            nms = sv.getModule();
        } else {
    ...
    private boolean setUpConfig() {
        ...
        Reader reader = TwiCosmeticsData.get().getPlugin().getFileReader("config.yml");
        ...
        config.set("Supported-Languages", supportedLanguages, "Languages supported by this version of TwiCosmetics.",
    ...
    public ProfileSaveService getProfileSaveService() {
        if (profileSaveService == null) {
            throw new IllegalStateException("Flat-file profile storage is not active");
        }
