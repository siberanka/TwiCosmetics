...
import com.siberanka.twicosmetics.player.profile.FileCosmeticsProfile;
...
public class UltraPlayer {
     ...
     * <p>
     * Kept as per-player state because gadget hit eligibility is session-scoped.
     */
     ...
     * @param ultraCosmetics TwiCosmetics
     */
    public UltraPlayer(UUID uuid, TwiCosmetics ultraCosmetics) {
        this.uuid = uuid;
    ...
    public boolean tryRemoveKey() {
        return cosmeticsProfile.tryRemoveKey();
    }
    ...
    public void addAmmo(GadgetType type, int amount) {
        if (!TwiCosmeticsData.get().isAmmoEnabled()) return;
    ...
    public void applyVelocity(Vector vector) {
        ...
        ultraCosmetics.getScheduler().runAtEntityLater(getBukkitPlayer(), () -> FallDamageManager.addNoFall(getBukkitPlayer()), 2);
    }
    ...
    public int getAmmo(GadgetType type) {
        if (!TwiCosmeticsData.get().isAmmoEnabled()) return 0;
