...
public class TreasureChestManager implements Listener {
    ...
    public TreasureChestManager(TwiCosmetics ultraCosmetics) {
    ...
    public boolean tryOpenChest(Player player, TreasureLocation tpTo) {
        ...
        String reservationKey = reservationKey(targetLoc);
        if (!occupiedLocations.add(reservationKey)) {
            MessageManager.send(player, "Treasure-Chest-Occupied");
            if (preLoc != null) ultraCosmetics.getScheduler().teleportAsync(player, preLoc);
            return false;
        }
        if (!ultraPlayer.tryRemoveKey()) {
            occupiedLocations.remove(reservationKey);
            if (preLoc != null) ultraCosmetics.getScheduler().teleportAsync(player, preLoc);
            return false;
        }
        ...
        player.closeInventory();
        try {
            new TreasureChest(player.getUniqueId(), new TreasureChestDesign(designPath), preLoc, tpTo, reservationKey);
    ...
    private String reservationKey(Location location) {
