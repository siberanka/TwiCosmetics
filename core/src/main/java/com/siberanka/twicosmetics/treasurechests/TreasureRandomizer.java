...
import com.siberanka.twicosmetics.treasurechests.loot.CommandLoot;
import com.siberanka.twicosmetics.treasurechests.loot.CommandLootContainer;
...
import com.siberanka.twicosmetics.treasurechests.loot.NothingLoot;
...
public class TreasureRandomizer {
    ...
    public TreasureRandomizer(final Player player, Location location, boolean forceMessageToOwner) {
        ...
        // add ammo.
        canAddAmmo = Category.GADGETS.isEnabled() && TwiCosmeticsData.get().isAmmoEnabled()
        ...
        if (TwiCosmeticsData.get().useMoneyTreasureLoot()) {
    ...
    private void initializeCommandLoot() {
        CustomConfiguration config = TwiCosmeticsData.get().getPlugin().getConfig();
    ...
    public LootReward giveRandomThing(TreasureChest chest, boolean skipFirework) {
        UltraPlayer ultraPlayer = TwiCosmeticsData.get().getPlugin().getPlayerManager().getUltraPlayer(player);
    ...
    private void broadcast(Component message, boolean toOthers) {
        ...
        TwiCosmetics ultraCosmetics = TwiCosmeticsData.get().getPlugin();
