...
public class PlayerBounceRunnable extends UltraTask {
    ...
    private final UltraPlayerManager pm = TwiCosmeticsData.get().getPlugin().getPlayerManager();

    public PlayerBounceRunnable(TreasureChest chest) {
        this.chest = chest;
        this.center = chest.getCenter();
        this.player = chest.getPlayer();
        this.ultraPlayer = TwiCosmeticsData.get().getPlugin().getPlayerManager().getUltraPlayer(player);
