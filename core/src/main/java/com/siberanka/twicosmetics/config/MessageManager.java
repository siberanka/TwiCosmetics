...
public class MessageManager {
    ...
    private final BukkitAudiences audiences = BukkitAudiences.create(TwiCosmeticsData.get().getPlugin());
    // should be set to true by the time anybody else can read this
    private boolean success = false;

    public static String getLangFilename() {
        return "messages_" + TwiCosmeticsData.get().getLanguage();
    }
    ...
    private MessageManager() {
        ...
        Reader reader = TwiCosmeticsData.get().getPlugin().getFileReader("messages/" + langFile + ".yml");
    ...
    private void addMessageInternal(String path, String message) {
        if (messagesConfig.addDefault(path, message)) {
            ...
            if (CosmeticType.GENERATE_MISSING_MESSAGES) {
                TwiCosmeticsData.get().getPlugin().getSmartLogger().write("Adding message " + path);
            }
        }
    }
