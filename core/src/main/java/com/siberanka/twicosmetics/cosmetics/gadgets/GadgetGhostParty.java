...
public class GadgetGhostParty extends Gadget implements Updatable {
    ...
    public GadgetGhostParty(UltraPlayer owner, GadgetType type, TwiCosmetics ultraCosmetics) {
    ...
    protected void onRightClick() {
        ...
        getTwiCosmetics().getScheduler().runLater(this::killBats, 160);
    }
    ...
    private void killBats() {
        if (getTwiCosmetics().getFoliaLib().isFolia()) {
            for (ArmorStand ghost : ghosts) {
                if (ghost.isValid()) {
                    getTwiCosmetics().getScheduler().runAtEntity(ghost, t -> ghost.remove());
                }
            }
        } else {
