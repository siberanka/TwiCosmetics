...
import com.siberanka.twicosmetics.cosmetics.type.CosmeticEntType;
...
public abstract class EntityCosmetic<T extends CosmeticEntType<?>, E extends Entity> extends Cosmetic<T> {
    ...
    public EntityCosmetic(UltraPlayer owner, T type, TwiCosmetics ultraCosmetics) {
    ...
    protected void removeEntitySafe(Entity entity) {
        if (entity != null) {
            if (getTwiCosmetics().isEnabled()) {
                getTwiCosmetics().getScheduler().runAtEntity(entity, t -> entity.remove());
            } else {
                try {
                    entity.remove();
