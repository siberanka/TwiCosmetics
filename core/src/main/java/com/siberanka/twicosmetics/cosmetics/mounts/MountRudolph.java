...
public class MountRudolph extends MountAbstractHorse {
    ...
    public MountRudolph(UltraPlayer owner, MountType type, TwiCosmetics ultraCosmetics) {
    ...
    private ArmorStand spawnArmorStand(boolean right) {
        ...
        armorStand.setMetadata("C_AD_ArmorStand", new FixedMetadataValue(getTwiCosmetics(), getPlayer().getUniqueId().toString()));
        return armorStand;
    }
    ...
    private void moveAntlers() {
        ...
        getTwiCosmetics().getScheduler().teleportAsync(left, location.clone().add(leftVector));
        getTwiCosmetics().getScheduler().teleportAsync(right, location.clone().add(rightVector));
        ...
        if (!TwiCosmeticsData.get().getVersionManager().isUsingNMS()) return;
        new Thread(() -> {
            for (Player player : getPlayer().getWorld().getPlayers()) {
                if (noseLocation.distanceSquared(player.getLocation()) > 32 * 32) continue;
                TwiCosmeticsData.get().getVersionManager().getEntityUtil().sendTeleportPacket(player, right);
                TwiCosmeticsData.get().getVersionManager().getEntityUtil().sendTeleportPacket(player, left);
            }
        }).start();
    }
