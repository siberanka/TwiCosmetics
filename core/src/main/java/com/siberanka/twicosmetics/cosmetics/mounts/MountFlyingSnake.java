...
public abstract class MountFlyingSnake extends Mount {
    ...
    public MountFlyingSnake(UltraPlayer owner, MountType type, TwiCosmetics ultraCosmetics, ItemStack tailItem) {
    ...
    public void onUpdate() {
        ...
        for (final ArmorStand armorStand : tail) {
            ...
            TwiCosmeticsData.get().getPlugin().getScheduler().teleportAsync(armorStand, lastLocation);
    ...
    private void summonTailPart(int j) {
        ...
        for (int i = 0; i < j; i++) {
            ...
            armorStand.setMetadata("NO_INTER", new FixedMetadataValue(getTwiCosmetics(), 1));
