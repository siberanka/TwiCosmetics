...
import com.siberanka.twicosmetics.cosmetics.pets.*;
...
public class PetType extends CosmeticEntType<Pet> {
    ...
    @Override
    public Pet equip(UltraPlayer player, TwiCosmetics ultraCosmetics) {
        Pet pet = super.equip(player, ultraCosmetics);
        if (pet != null && customization != null) {
            if (!pet.setCustomization(customization)) {
                TwiCosmeticsData.get().getPlugin().getSmartLogger().write(LogLevel.WARNING, "Invalid customization string for pet " + getConfigName());
    ...
    public static void register() {
        ...
        if (TwiCosmeticsData.get().getVersionManager().isUsingNMS()) {
            registerPet("Pumpling", XMaterial.PUMPKIN, XEntityType.ZOMBIE, TwiCosmeticsData.get().getVersionManager().getModule().getPumplingClass());
        }
        ...
        SmartLogger log = TwiCosmeticsData.get().getPlugin().getSmartLogger();
