...
import com.siberanka.twicosmetics.run.MountRegionChecker;
...
public abstract class Mount extends EntityCosmetic<MountType, Entity> implements Updatable {
    ...
    public Mount(UltraPlayer ultraPlayer, MountType type, TwiCosmetics ultraCosmetics) {
    ...
    public void onEquip() {
        ...
        getTwiCosmetics().getPaperSupport().setCustomName(entity, getType().getName(getPlayer()));
        ...
        if (!getTwiCosmetics().getWorldGuardManager().isHooked()) return;
        // Horses trigger PlayerMoveEvent so the standard WG move handler will be sufficient
        if (isHorse(entity.getType())) return;
        mountRegionTask = new MountRegionChecker(getOwner(), getTwiCosmetics());
    ...
    public void onEntityDamage(EntityDamageEvent event) {
        if (event.getEntity() == getPlayer() && getOwner().getCurrentMount() == this
                && !getTwiCosmetics().getConfig().getBoolean("allow-damage-to-players-on-mounts")) {
    ...
    protected void onPortal() {
        ...
        getTwiCosmetics().getScheduler().runAtEntityLater(getEntity(), this::onEquip, 1);
    }
