...
public class GadgetPaintballGun extends Gadget {
    ...
    static {
        ...
        if (PAINT_BLOCKS.isEmpty()) {
            TwiCosmeticsData.get().getPlugin().getSmartLogger().write(LogLevel.ERROR, "Paintball Gun setting 'Block-Type' does not match any known blocks.");
    ...
    public GadgetPaintballGun(UltraPlayer owner, GadgetType type, TwiCosmetics ultraCosmetics) {
    ...
    public void onItemFrameBreak(HangingBreakByEntityEvent event) {
        if (projectiles.contains(event.getRemover())) {
            ...
            // Paintball projectiles are cosmetic and must never break hanging entities.
