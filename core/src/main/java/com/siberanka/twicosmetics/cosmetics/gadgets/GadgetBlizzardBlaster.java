...
public class GadgetBlizzardBlaster extends Gadget implements PlayerAffectingCosmetic, Updatable {
    ...
    public GadgetBlizzardBlaster(UltraPlayer owner, GadgetType type, TwiCosmetics ultraCosmetics) {
    ...
    protected void onRightClick() {
        ...
        getTwiCosmetics().getScheduler().runAtEntityLater(getPlayer(), () -> active = false, 40);
    ...
    public void onUpdate() {
        if (active) {
            ...
            for (int i = 0; i < 3; i++) {
                TwiCosmeticsData.get().getVersionManager().getEntityUtil().sendBlizzard(getPlayer(), location, entity -> canAffect(entity, getPlayer()), vector);
            }
    ...
    public void onClear() {
        ...
        TwiCosmeticsData.get().getVersionManager().getEntityUtil().clearBlizzard(getPlayer());
    }
}
