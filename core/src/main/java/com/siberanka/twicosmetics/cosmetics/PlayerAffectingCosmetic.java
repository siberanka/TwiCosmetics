...
public interface PlayerAffectingCosmetic {
    public default boolean canAffect(Entity entity, Player owner) {
        ...
        if (entity instanceof Player) {
            ...
            UltraPlayer ultraPlayer = getSelf().getTwiCosmetics().getPlayerManager().getUltraPlayer(target);
            if (!ultraPlayer.canBeHitByOtherGadgets() || !ultraPlayer.hasGadgetsEnabled() || ultraPlayer.getCurrentTreasureChest() != null) {
                return false;
            }
            if (!getSelf().getTwiCosmetics().getWorldGuardManager().canAffectPlayersHere(target)) {
                return false;
            }
        } else if (entity instanceof Creature && !TwiCosmeticsData.get().isCosmeticsAffectEntities()) {
            return false;
