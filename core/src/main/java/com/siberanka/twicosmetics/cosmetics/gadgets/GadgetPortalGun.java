...
import com.siberanka.twicosmetics.util.PortalLoc;
...
public class GadgetPortalGun extends Gadget implements PlayerAffectingCosmetic, Updatable {
    ...
    public GadgetPortalGun(UltraPlayer owner, GadgetType type, TwiCosmetics ultraCosmetics) {
    ...
    private boolean portalTeleportCheck(Player player, PortalLoc portalLoc, PortalLoc dest) {
        ...
        getTwiCosmetics().getScheduler().runAtEntityLater(getPlayer(), () -> playersOnCooldown.remove(player.getUniqueId()), 20);
        return true;
    }
    ...
    private void teleport(final Entity entity, final Location location, final Vector velocity) {
        getTwiCosmetics().getScheduler().runNextTick((task) -> {
            getTwiCosmetics().getScheduler().teleportAsync(entity, location);
